...
rx_ic_m = fact.groupby(["MONTH_OF_SUPPLY", "ITEM_CODE"], as_index=False)[
    "PRESCRIPTIONS"
].sum()
...
condition = condition.groupby(["item-code"], as_index=False)["condition"].apply(
    lambda x: ",".join(x)
)
...
if not (drug_name):
    ...
    fcst_date_range = pd.date_range(
        start=last_mth_of_actuals, end=fcst_till_period, freq="MS", closed="right"
    )

    x_fcst = np.array(
        [i for i in range(len(x) + 1, len(x) + 1 + len(fcst_date_range))], dtype="float"
    )
    y_fcst = np.round(np.log(x_fcst) * fit[0] + fit[1], 0)

    # create forecast for lookback period to show how accurate the forecast is
    x_lbp_fcst = np.array(
        [i for i in range(len(x) + 1 - lbp, len(x) + 1)], dtype="float"
    )
    y_lbp_fcst = np.round(np.log(x_lbp_fcst) * fit[0] + fit[1], 0)
    lbp_fcst_date_start = df2["MONTH_OF_SUPPLY"].sort_values().unique()[-lbp]
    lbp_fcst_date_range = pd.date_range(start=lbp_fcst_date_start, 
                                        end=last_mth_of_actuals, 
                                        freq="MS", 
                                        closed=None)
    ...
    ax.plot(lbp_fcst_date_range, y_lbp_fcst, ls=":", color="grey")
    ...
    st.markdown(
        "- If you expect changes in the market or competition, then feel free to download the chart data and make adjustments accordingly."
    )
    st.write("#")
    st.download_button(
        label="Download data",
        data=pd.DataFrame(final_y, final_x, columns=["Prescriptions"])
        .transpose()
        .to_csv(),
